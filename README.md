...
このワークフローは、確認元リポジトリへの変更を自動的に検出し、整合性チェックを実行します。リポジトリの状態を確認し、必要な処理を行います。

**注意**: ワークフローファイル（`.github/workflows/backup-sync.yml`）は確認元リポジトリにのみ配置してください。チェック先リポジトリには配置する必要はありません。
...
**注意**: チェック先リポジトリにはワークフローファイルを配置する必要はありません。確認元リポジトリにのみ配置してください。
